...
- **插件市场（可视化面板）**：搜索 npm、标注是否真 dsh 插件（`dsh.bundle`）、**卡片显示 GitHub ⭐ 星数与 npm 月下载量，支持按 星数/下载量 排序**，可"只看 dsh 插件"（主进程真实过滤+翻页）；安装前 peer 兼容分析、一键安装/卸载、**重启内核即生效**；插件安装在 `$DSH_HOME/profiles/web`（与 `dsh web` 共用）。
